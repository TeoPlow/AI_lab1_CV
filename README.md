...
## 2. Реализация (аналог пунктов 2-4 из ЛР6, но на Ultralytics YOLOv11)
...
### 3.8 Демонстрационный notebook (опционально)

Для наглядной демонстрации результатов доступен notebook:
- `Lab1_Demo.ipynb`

В нем:
- автоматическая проверка наличия модели и артефактов;
- чтение итоговых метрик из JSON;
- визуализация динамики обучения из `results.csv`;
- запуск инференса на части test изображений;
- отображение предсказаний прямо в notebook.

### 3.9 Итог текущего запуска (готовые артефакты)

Для версии датасета `v8 (raw_HelmetClassOnly)` в этом репозитории уже получены результаты:
- train run dir: `runs/train/helmet_detector_gpu_final`
- best weights: `runs/train/helmet_detector_gpu_final/weights/best.pt`
- метрики: `runs/train/helmet_detector_gpu_final/metrics_summary.json`
- инференс: `runs/predict/helmet_detector_gpu_final`
...
## 4. Что показать в отчете

Минимум для варианта на 3:
- обоснование выбора датасета и метрик (раздел 1);
- 1 базовый запуск обучения (YOLO11n);
- метрики на test split: mAP50-95, mAP50, Precision, Recall;
- несколько примеров инференса (скриншоты из `runs/predict/...`);
- короткое сравнение 2 конфигураций (например, YOLO11n vs YOLO11s).

Примечание:
- для варианта на тройку достаточно одного корректного воспроизводимого запуска (YOLO11n) с обоснованными метриками и примерами предсказаний.

## 5. Состав репозитория

- `dataset/` - локальный датасет
- `src/lab1_yolo11.py` - скрипт пайплайна
- `Lab1_Demo.ipynb` - демонстрационный notebook с метриками и визуализацией
- `requirements.txt` - зависимости
- `README.md` - инструкция и обоснование
